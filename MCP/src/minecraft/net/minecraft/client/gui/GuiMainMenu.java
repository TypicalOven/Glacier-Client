...
public class GuiMainMenu extends GuiScreen implements GuiYesNoCallback
{
    ...
    public void drawScreen(int mouseX, int mouseY, float partialTicks)
    {
        ...
        DrawUtil.draw2DImage(new ResourceLocation("glacier/main-menu.png"),0, 0, this.width, this.height, Color.WHITE);
        DrawUtil.draw2DImage(new ResourceLocation("glacier/logo.png"), this.width / 2 - 100, this.height / 4 - 8, 60, 60, Color.WHITE);
        FontUtil.bold_large.drawStringWithShadow("Glacier Client", this.width / 2 - 45, this.height / 4 + 18, -1);
        FontUtil.bold.drawStringWithShadow("v" + Glacier.INSTANCE.VERSION, this.width / 2 + 65, this.height / 4 + 15, -1);
        Tessellator tessellator = Tessellator.getInstance();
        WorldRenderer worldrenderer = tessellator.getWorldRenderer();
        int i = 274;
       ...
       // Gui.drawRect(5, 5, 255, 78, new Color(0, 0, 0, 100).getRGB());
        fontRendererObj.drawStringWithShadow("Glacier Client version 1.1 (Content update 1)", 8, 8, -1);

        ArrayList<String> changelog = new ArrayList<>();

        changelog.add("[+] Added Discord RPC");
        changelog.add("[+] Added networking with the Glacier Network. Users will now");
        changelog.add("be able to see each other's cosmetics.");
        changelog.add("[*] Fixed bug where users were unable to drag mods");
        changelog.add("[*] Fixed bug where ToggleSprint was not using correct key");
        changelog.add("[*] Fixed bug where users could not save or load profiles");
        changelog.add("[*] Fixed bug where jump key was not working in Keystrokes");
        changelog.add("[*] Fixed crashes on the resource pack screen");
        int y = 25;
        GlStateManager.pushMatrix();
        GlStateManager.scale(0.8, 0.8, 0.8);
        for(String s : changelog) {
            if(s.startsWith("[+]")) s = s.replace("[+]", "[" + EnumChatFormatting.GREEN + "+" + EnumChatFormatting.RESET + "]");
            if(s.startsWith("[*]")) s = s.replace("[*]", "[" + EnumChatFormatting.YELLOW + "*" + EnumChatFormatting.RESET + "]");
            if(s.startsWith("[-]")) s = s.replace("[-]", "[" + EnumChatFormatting.RED + "-" + EnumChatFormatting.RESET + "]");
            fontRendererObj.drawStringWithShadow(s, 10, y, -1);
            y += 10;
        }
        GlStateManager.popMatrix();
        ...
        this.fontRendererObj.drawStringWithShadow(s2, this.width - this.fontRendererObj.getStringWidth(s2) - 2, this.height - 10, -1);
